...
echo ("<FONT SIZE=4><CENTER><B>Розклад занять для викладача $prepod.</B></CENTER></FONT><hr>");
#========================================================================================
$denn='Понеділок';
$zapros = mysql_query("SELECT * FROM rozklad WHERE prepod1='$prepod' AND dennedeli='$denn' order by para asc");
$res1 = mysql_fetch_array($zapros);
if ($res1[para]<>'') {
echo ("<FONT SIZE=4><B>$denn</B></FONT><br>");
echo ("<table width='100%' border='1' cellspacing='0' cellpadding='0'>");
echo ("<tr><td><B>Пара</B></td><td><B>Час</B></td><td><B>Парність</B></td><td><B>Предмет</B></td><td><B>Група</B></td><td><B>Робочі тижні</B>");
echo ("</td><td><B>Ауд</B></td><td><B>Примітки</B></td></tr>");
$zapros = mysql_query("SELECT * FROM rozklad WHERE prepod1='$prepod' AND dennedeli='$denn' order by para asc");
while ($res2 = mysql_fetch_array($zapros))
{
$gru=urldecode($res2[grupa]);
echo ("<tr><td>$res2[para]</td><td>$res2[chas]</td><td>$res2[parnist]</td><td>$res2[predmet]</td><td>$gru</td><td>$res2[tizniroku]
